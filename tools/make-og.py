...
/* cavendish coworking — THE FRONT OF THE HOUSE, LIFTED WHOLE: the header with its
   drawing of a Georgian house front, the door shut in its Coade stone case and
...
.og--cavendish h1 {{ font-size: 74px; margin: 4px 0 6px !important; }}
.og--cavendish .og-lede {{ order: 4; font-family: 'Libre Caslon Text', Georgia, serif; color: var(--cw-cream);
  max-width: 1060px; font-size: 24px; line-height: 1.4; margin: 14px 0 0 !important; }}
.og--cavendish .cw-art {{ order: 5; margin: 14px 0 0 !important; }}
.og--cavendish .cw-front {{ width: 1080px; }}
...
def card_cavendish(p):
    ...
    # alt counts no doors: they are the events page's and there will be more.
    ...
    if not (head and over and sub and 'class="cw-front"' in head.group(1)):
        raise SystemExit(
            "REFUSING: cavendish-coworking.html has lost its header or its drawing of the\n"
            "house front, and the card is those two. Redesign the card on purpose.")
    plain = lambda h: html.unescape(re.sub(r"<[^>]+>", "", h)).strip()
    block = head.group(1).replace("</header>", f'<p class="og-lede">{p["desc"]}</p></header>')
    return (
        "",
        f'<div class="og og--cavendish" data-fit="card">{block}</div>',
        f"A card in a deep Georgian green, the colour of a painted hall. Small pale capitals "
        f"reading {plain(over.group(1))}, then \u201c{p['h1text']}\u201d in a large old-style "
        f"serif in cream, then {plain(sub.group(1))}. Under it: {p['desc_plain']} Across the "
        f"bottom, a drawing of the front of a Georgian brick house: a six-panel mahogany door, "
        f"shut, with a brass knocker, in a pale stone case with a rounded fanlight over it and "
        f"steps up to it; tall sash windows with wrought-iron balconies on the floor above; and "
        f"in every window a green canvas blind drawn down, with shelves of books showing "
        f"through it. Iron railings run along the front. Nobody is drawn.",
    )
